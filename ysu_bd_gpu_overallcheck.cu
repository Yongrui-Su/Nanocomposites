...
sim.t0max = 800;           //msd(t) the maximum number of initial conditions
  read_sim();
  initial_all();
  system_out();
...
if(MSD == true){
    ...
    if(sim.Nano_num > 0)nano_ntime = (double *)calloc(sim.tmax,sizeof(double));
    ...
    if(sim.Nano_num > 0){
      nano_r2f = (double *)calloc(sim.tmax,sizeof(double));
      nano_dx = (double *)calloc(sim.tmax,sizeof(double));
      nano_dy = (double *)calloc(sim.tmax,sizeof(double));
      nano_dz = (double *)calloc(sim.tmax,sizeof(double));
    }
    chain_r2f = (double *)calloc(sim.tmax,sizeof(double));

    time0 = (double *)calloc(sim.t0max,sizeof(double));
    if(sim.Nano_num > 0)nano_time0 = (double *)calloc(sim.t0max,sizeof(double));
    ...
    if(sim.Nano_num > 0){
      nano_X0=(double *)calloc(sim.Nano_num*sim.t0max,sizeof(double));
      nano_Y0=(double *)calloc(sim.Nano_num*sim.t0max,sizeof(double));
      nano_Z0=(double *)calloc(sim.Nano_num*sim.t0max,sizeof(double));
    }
  }
...
void nano_msd(int sw){
  ...
  case 0:
    ...
    break;
  case 1:
    if(++nano_ntel%sim.it0 ==0 && nano_t0<sim.t0max)
      {
	nano_time0[nano_t0] = nano_ntel;
	for(int i =0;i < sim.Nano_num;i++){
	  nano_X0[i*sim.t0max+nano_t0] = nano_v[i].x;
	  nano_Y0[i*sim.t0max+nano_t0] = nano_v[i].y;
	  nano_Z0[i*sim.t0max+nano_t0] = nano_v[i].z;
	}
	nano_t0++;
      }
	...
	for(int j=0;j<sim.Nano_num;j++){
	  dx = nano_v[j].x-nano_X0[j*sim.t0max+i];
	  dy = nano_v[j].y-nano_Y0[j*sim.t0max+i];
	  dz = nano_v[j].z-nano_Z0[j*sim.t0max+i];
  ...
  case 2:
    ...
    io1 = fopen(buff,"w");
    for(int i=0;i<sim.tmax;i++)if(nano_ntime[i] == 0)break;
...
void save_check()
{
 ...
 if(MSD == true){
    ...
    if(sim.Nano_num>0)fwrite(&nano_ntime[0],sizeof(double),sim.tmax,io);
    ...
    if(sim.Nano_num>0){
      fwrite(&nano_r2f[0],sizeof(double),sim.tmax,io);
      fwrite(&nano_dx[0],sizeof(double),sim.tmax,io);
      fwrite(&nano_dy[0],sizeof(double),sim.tmax,io);
      fwrite(&nano_dz[0],sizeof(double),sim.tmax,io);
    ...
    if(sim.Nano_num>0)fwrite(&nano_time0[0],sizeof(double),sim.t0max,io);
    ...
    if(sim.Nano_num>0){
      fwrite(&nano_X0[0],sizeof(double),sim.t0max*sim.Nano_num,io);
      fwrite(&nano_Y0[0],sizeof(double),sim.t0max*sim.Nano_num,io);
      fwrite(&nano_Z0[0],sizeof(double),sim.t0max*sim.Nano_num,io);
    }
    fclose(io);
  }
...
void read_check()
{
  ...
  if (io==NULL)
	...
	if(sim.Nano_num>0)fread(&nano_ntime[0],sizeof(double),sim.tmax,io);
	...
	if(sim.Nano_num>0){
	  fread(&nano_r2f[0],sizeof(double),sim.tmax,io);
	  fread(&nano_dx[0],sizeof(double),sim.tmax,io);
	  fread(&nano_dy[0],sizeof(double),sim.tmax,io);
	  fread(&nano_dz[0],sizeof(double),sim.tmax,io);\
	...
	if(sim.Nano_num>0)fread(&nano_time0[0],sizeof(double),sim.t0max,io);
	...
	if(sim.Nano_num>0){
	  fread(&nano_X0[0],sizeof(double),sim.t0max*sim.Nano_num,io);
	  fread(&nano_Y0[0],sizeof(double),sim.t0max*sim.Nano_num,io);
	  fread(&nano_Z0[0],sizeof(double),sim.t0max*sim.Nano_num,io);
	}
	fclose(io);
      }
    }
